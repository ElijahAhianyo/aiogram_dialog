...
<!---Documatic-section-aiogram_dialog.dialog.Dialog-start--->
## aiogram_dialog.dialog.Dialog

<!---Documatic-section-Dialog-start--->
<!---Documatic-block-aiogram_dialog.dialog.Dialog-start--->
<details>
	<summary><code>aiogram_dialog.dialog.Dialog</code> code snippet</summary>

```python
class Dialog(ManagedDialogProto):

    def __init__(self, *windows: DialogWindowProto, on_start: Optional[OnDialogEvent]=None, on_close: Optional[OnDialogEvent]=None, on_process_result: Optional[OnResultEvent]=None, launch_mode: LaunchMode=LaunchMode.STANDARD, getter: GetterVariant=None, preview_data: GetterVariant=None):
    ...
    async def switch_to(self, state: State, manager: DialogManager):
        if state.group != self.states_group():
            raise ValueError(f'Cannot switch from `{self.states_group_name()}` to another states group {state.group}')
    ...
    async def _current_window(self, manager: DialogManager) -> DialogWindowProto:
        try:
            ...
            raise UnregisteredWindowError(f'No window found for `{manager.current_context().state}` Current state group is `{self.states_group_name()}`') from e
    ...
    async def show(self, manager: DialogManager) -> None:
        ...
        if new_message.media and (not new_message.media.file_id):
            new_message.media.file_id = await media_id_storage.get_media_id(path=new_message.media.path, url=new_message.media.url, type=new_message.media.type)
        ...
        if new_message.media:
            await media_id_storage.save_media_id(path=new_message.media.path, url=new_message.media.url, type=new_message.media.type, media_id=get_media_id(message))
    ...
    async def _message_handler(self, m: Message, dialog_manager: DialogManager):
        ...
        if dialog_manager.current_context() == intent:
...
<!---Documatic-block-aiogram_dialog.dialog.Dialog-end--->
<!---Documatic-section-Dialog-end--->

# #
<!---Documatic-section-aiogram_dialog.dialog.Dialog-end--->
